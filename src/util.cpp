...
const char * const BITCOIN_CONF_FILENAME = "bitbony.conf";
const char * const BITCOIN_PID_FILENAME = "bitbonyd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "bitbony" is a composite category enabling all BitBony-related debug output
            if(ptrCategory->count(string("bitbony"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitBonyCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitBonyCore
    // Mac: ~/Library/Application Support/BitBonyCore
    // Unix: ~/.bitbonycore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitBonyCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitBonyCore";
#else
    // Unix
    return pathRet / ".bitbonycore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty bitbony.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override bitbony.conf
